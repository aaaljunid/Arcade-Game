...
var Enemy = function(x, y, fast) {
    //  This Variables (x, y) for axis and (fast) for determine the speed,
    this.x = x;
    this.y = y;
    this.fast = fast;
...
Enemy.prototype.update = function(dt) {
    // this code for multiply any movement by the dt parameter
    // which will ensure the game runs at the same speed for
    // all computers.
    this.x += this.fast * dt;

    //when the enemies are leave the canvas,
    // they reappear randomly with different speeds
    if (this.x >= 510) {
        this.x = -50;
        this.fast = 100 + Math.floor(Math.random() * 400);
    };


    // Checks for crash between the player and the enemies
...
// This class for the player function   
var Player = function (x, y) {

    // Variables for player to move along x and y axis 
    this.x = x;
    this.y = y;

    //The image of The player's character is princess-girl which added to the playing field 
